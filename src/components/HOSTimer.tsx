...
export default function HOSTimer({ tripActive, onTick }: Props) {
  ...
  return (
    <div className={`rounded-2xl p-5 ${isDanger ? "bg-red-900" : isWarning ? "bg-yellow-900" : "bg-gray-800"}`}>
      ...
      <p className="text-gray-400 text-sm mt-1">Max: {MAX_DRIVING_HOURS} hrs</p>
      ...
      {isDanger && <p className="text-red-400 font-bold mt-3 text-center animate-pulse">🚨 LIMIT EXCEEDED — PULL OVER NOW</p>}
      {isWarning && !isDanger && <p className="text-yellow-400 font-semibold mt-3 text-center">⚠️ 80% limit reached — plan rest stop</p>}
    </div>
  )
}